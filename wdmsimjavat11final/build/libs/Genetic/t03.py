...
def convert_trace_to_excel(trace_file_path, output_file):
    ...
    with open(trace_file_path, 'r') as file:
        for line in file:
            ...
            if line.startswith("lightpath-created") or line.startswith("lightpath-removed"):
                event = parts[0]
                lp_id = parts[1]
                src = parts[2]
                dst = parts[3]
                # Exclude the "Link_Wavelength" column by not adding it
                lightpaths_data.append([event, lp_id, src, dst])

    # Convert parsed data to dataframe
    lightpaths_df = pd.DataFrame(lightpaths_data, columns=["Event", "ID", "Src", "Dst"])
...
def plot_data():
    ...
    ax1.plot(loads, total_blocked, marker='o', color='g', label='Total Blocked')
    ax1.set_xlabel('Load')
    ...
    ax3.bar(loads, lightpath_counts, color='g', label='Lightpath Created Count', width=1.5)  # Thicker bars
    ax3.set_xlabel('Load')
